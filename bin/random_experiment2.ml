...
let () =
  try
    ...
    | [v; p; s; sim] ->
      ...
      let p_size = int_of_string p in
      let s_size = int_of_string s in
      let similarity = float_of_string sim in

      (* Generates variable pool *)
      let var_pool = generate_var_pool v_size in

      (* Generates main heap *)
      let big_heap = rand_heap var_pool p_size s_size in

      (* Generates two smaller heaps similar to the one passed. *)
      let (heap1, heap2) = generate_smaller big_heap similarity var_pool in

      if !original_setting then (
        print_endline ("\nOriginal heap:  " ^ (format_symb_heap heap1));
      );
    ...
    | _ ->
      Printf.eprintf "Expected: <v_size> <p_size> <s_size> <similarity>\n";
      exit 1
