...
let test_symb_heap_format _ =
  ...
  let sh1 = {exists = []; pure = [Comp(Eq, Int 2, Int(2))]; spatial = [PointsTo(x, Int(5))]} in
  let sh2 = {exists = []; pure = [Comp(Neq, v0, Int(2))]; spatial = [Freed(y); PointsTo(x, Int(5))]} in
  let sh3 = {exists = [1]; pure = [Comp(Eq, e, f); Comp(Neq, v1, Int(42))]; spatial = [PointsTo(x, Int(6)); PointsTo(e, Int(5)); PointsTo(y, Int(7))]} in
  let sh4 = {exists = []; pure = [Comp(Eq, Int 2, Ide(Lvar(fresh_lvar())))]; spatial = [PointsTo(w, Int(5)); PointsTo(z, Int(5)); List(k, Int(7))]} in
  let sh5 = {exists = [0; 1]; pure = [Comp(Neq, Ide(Pvar "x"), v1)]; spatial = [PointsTo(v0, Int(5)); Freed(f)]} in
...
let sp1 = TrueS;;
let sp2 = PointsTo(x, e1);;
let sp3 = List(y, z);;
let sp4 = Freed(w);;


let sh1 = empty_sh;;
let sh2 = {pure = [Comp(Eq, e1, Int(2))]; spatial = [PointsTo(x, Int(5))]};;
let sh3 = {pure = [Comp(Neq, Ide(Lvar(fresh_lvar ())), Int(2))]; spatial = [Freed(y); PointsTo(x, Int(5))]};;
let sh4 = {pure = [Comp(Eq, Ide(e), Ide(f))]; spatial = [PointsTo(x, Int(6)); PointsTo(e, Int(5)); PointsTo(y, Int(7))]};;
let sh5 = {pure = [pp2]; spatial = [PointsTo(w, Int(5)); PointsTo(z, Int(5)); PointsTo(k, Int(7))]};;
let sh6 = {pure = [pp3]; spatial = [PointsTo(Lvar(fresh_lvar ()), Int(5))]};;

print_endline "\nTesting symbolic heaps formatting, | divides the pure predicates from the spatial predicates";;
print_endline (format_symb_heap sh1);;
print_endline (format_symb_heap sh2);;
print_endline (format_symb_heap sh3);;
print_endline (format_symb_heap sh4);;
print_endline (format_symb_heap sh5);;
print_endline (format_symb_heap sh6);;
